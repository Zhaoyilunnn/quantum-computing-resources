...
class Opt_Classifier(paddle_quantum.gate.Gate):
    ...
    def forward(self, state_in, label):
        ...
        # 映射 <Z> 处理成标签的估计值
        state_predict = (
            paddle.real(E_Z)[:, 0] * 0.5 + 0.5 + self.bias
        )  # 计算每一个y^{i,k}与真实值得平方差
        loss = paddle.mean(
...
def heatmap_plot(Opt_Classifier, N):
    ...
    # 计算预测: heat_data
    input_state_test = paddle.to_tensor(datapoints_transform_to_state(x_y_, N))
    loss_useless, acc_useless, state_predict, cir = Opt_Classifier(
        state_in=input_state_test, label=x_y_[:, 0]
    )
